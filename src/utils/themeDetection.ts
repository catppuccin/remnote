 ...
 */
export function setupThemeDetection(plugin: ReactRNPlugin): void {
 ...
 */
function handleAppearanceChange(plugin: ReactRNPlugin) {
  return async (newValue: any) => {
    const isDarkMode: boolean = newValue.darkMode;
    console.log(`Dark mode changed via AppEvents: ${isDarkMode ? "dark" : "light"}`);
